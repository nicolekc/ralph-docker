...
- **`REPO_MEM_DIR/MEMORY.md` index (+ memory files)** — these go hand-in-hand:
  - If memories were found in the built-in location: create `MEMORY.md` with an index entry per migrated memory, plus the individual memory files themselves. List them by name with a summary of each (1–3 sentences; short=1, long=3) before proposing migration, with the tradeoff: leave them orphaned in `~/.claude/` and invisible to the repo, or move them in.
